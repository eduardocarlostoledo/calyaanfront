...
const ScheduleProfessional = ({ loadingLiberar, nombreServicio, localidadServicio, reserva, setReserva, id, guardarReserva }) => {
  ...
  const handleSubmitProfessional = async () => {
    ...
    try {
      const { data } = await clienteAxios.post("api/pay/finish/agendar", { id, profesional_id: profesional.creador._id, cita_servicio: inputValue.date, hora_servicio: inputValue.time });
  ...
  return (
            ...
            >
              <div className="flex flex-col items-center p-6 max-lg:my-3">
                ...
                <h5 className="mb-1 text-xl font-medium text-gray-900">
                  {reserva.profesional_id?.nombre}   {reserva.profesional_id?.apellido}
                </h5>
    ...
    (
                      ...
                      (reserva.metodo_pago && !reserva.cita_servicio && !reserva.hora_servicio) ? 
                      //then
                      (
                        ...
                        profesionalesRequest.length > 0 ? (
                          profesionalesRequest.map((profesionalMap) => (
                            <div
                              ...
                              className={`w-full max-w-sm rounded-lg shadow-md ${profesionalMap.styles
                                ? "bg-gray-100 border border-gray-200"
                                : ""
                                }`}
                            >
                              <div className="flex flex-col items-center p-6 max-lg:my-3">
                                ...
                                <div className="flex mt-4 space-x-3 md:mt-6">
                                  <div
                                    ...
                                    className={`inline-flex items-center px-4 py-2 text-sm font-medium text-center cursor-pointer text-white bg-primary rounded-lg hover:bg-bgHover focus:ring-4 focus:outline-none focus:ring-bgHover ${hourSelect.length > 0 && "hidden"
                                      }`}
                                  >
                                    Seleccionar
                                  </div>
